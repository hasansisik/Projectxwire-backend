...
const register = async (req, res, next) => {
  try {
    ...
    //check company
    const companyAlreadyExists = await Company.find({ companyId });
    if (!companyAlreadyExists) {
      throw new CustomError.BadRequestError("Kayıtlı şirket yok.");
    }
...
const login = async (req, res, next) => {
  try {
    ...
    if (user.company.toString() !== companyId) {
      throw new CustomError.UnauthenticatedError(
        "Bu kullanıcı belirtilen şirkete ait değil."
      );
    }
